...
def run_with_metrics(use_model=True, run_duration=120):
    ...
    while traci.simulation.getTime() - start_time < run_duration:
        ...
        if step % 50 == 0:
            avg_waiting = sum(metrics_history['waiting_times'][-50:]) / min(50, len(metrics_history['waiting_times']))
            avg_speed = sum(metrics_history['speeds'][-50:]) / min(50, len(metrics_history['speeds']))
            avg_throughput = sum(metrics_history['throughputs'][-50:]) / min(50, len(metrics_history['throughputs']))
            
            print(f"   Step {step}: Espera={avg_waiting:.1f}s, Velocidad={avg_speed:.2f}m/s, Throughput={avg_throughput:.1f}")
    ...
    final_metrics = {
        'avg_waiting': sum(metrics_history['waiting_times']) / len(metrics_history['waiting_times']),
        'avg_speed': sum(metrics_history['speeds']) / len(metrics_history['speeds']),
        'avg_throughput': sum(metrics_history['throughputs']) / len(metrics_history['throughputs'])
    }
